  ...
  context "active_years" do
    ...
    it "has assumptions" do
      ...
      expect(roles).to match_array [
        ["Group::VereinJodler::Conductor", groups(:jodlerklub_berna_bern).id]
      ]
    end
    ...
    it "considers Mitglied" do
      create_role(Group::VereinJodler::Mitglied, years: 10)
      subject.update_active_years
    ...
    context "assuming it is the summer of 2020" do
      ...
      it "counts completed years in the past" do
        expect do
          create_role(Group::VereinJodler::Mitglied, start_date: "2018-12-01", end_date: nil)
          subject.update_active_years
      ...
      it "counts only years with membership" do
        expect do
          create_role(Group::VereinJodler::Mitglied, start_date: "2018-02-01", end_date: "2018-04-30")
          create_role(Group::VereinJodler::Mitglied, start_date: "2018-12-01", end_date: "2019-07-31")
          subject.update_active_years
        end.to change(subject, :active_years).to(2)
      end
      ...
      it "counts multiple durations in one year only once" do
        expect do
          create_role(Group::VereinJodler::Mitglied, start_date: "2018-02-01", end_date: "2018-04-30")
          create_role(Group::VereinJodler::Mitglied, start_date: "2019-01-01", end_date: nil)
          subject.update_active_years
        end.to change(subject, :active_years).to(2)
      end
      ...
      it "counts years which have an short interruption fully" do
        expect do
          create_role(Group::VereinJodler::Mitglied, start_date: "2018-02-01", end_date: "2018-04-30")
          create_role(Group::VereinJodler::Mitglied, start_date: "2019-01-01", end_date: "2019-10-31")
          create_role(Group::VereinJodler::Mitglied, start_date: "2020-01-01", end_date: nil)
          subject.update_active_years
        end.to change(subject, :active_years).to(2)
      end
    ...
    it "handles active_years not being cached" do
      travel_to("2020-03-16") do
        expect(subject.roles.with_inactive.where(type: "Group::VereinJodler::Mitglied").count).to eq 0
        expect(subject.active_years).to be_nil

        expect(subject.prognostic_active_years).to eq 0

        Fabricate(
          Group::VereinJodler::Mitglied.name.to_sym,
          person: subject,
          group: groups(:jodlergruppe_engstligtal_adelboden),
        ...
        expect(subject.roles.where(type: "Group::VereinJodler::Mitglied").count).to eq 1
